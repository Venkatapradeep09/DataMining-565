...
//			System.out.println(decisionTree.getTheBestAttributeForSpilliting(dataRecords));
			decisionTree.buildTree(inputData.dataRecords);
...
//		System.out.println(numberOfHits);
	...
	public void buildTree(List<DataRecord> dataRecords){
		if(root == null)
			root = new DecisionNode();
		this.constructTreeRecursively( dataRecords , root);
	}
	
	private void constructTreeRecursively(List<DataRecord> dataRecords,
			DecisionNode root2) {
		...
		if(this.stopping_build_tree(dataRecords) == true){
			...
			for( String key: classLableCountValue.keySet()){
//				System.out.println( key + ":" + classLableCountValue.get(key));
			}
			root2.setClassLable(max_count_key);
//			System.out.println(max_count_key);
			return;
		}else{
			...
			System.out.println(dataRecordsLeft.size()+ "_" + dataRecordsRight.size());
